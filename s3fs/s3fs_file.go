...
//
// If n >= 1, Readdir returns at most n FileInfo structures from the directory. In this case, if
// Readdir returns an empty slice, it will return a non-nil error
// explaining why. At the end of a directory, the error is io.EOF.
//
// If n == 0,-1 Readdir returns all the FileInfo from the directory in
...
func (f *File) Readdir(n int) ([]os.FileInfo, error) {
	...
	for _, fileObject := range response.Contents {
		...
		name := *fileObject.Key
		if n > -2 {
			name = path.Base("/" + *fileObject.Key)
		}
		fis = append(fis, NewFileInfo(name, false, *fileObject.Size, *fileObject.LastModified))
	}
