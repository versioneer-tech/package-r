...
| --- | --- |
| `FB_DATABASE` | File Browser database path. Defaults to `./filebrowser.db`, matching plain `./filebrowser` startup. |
| `FB_ROOT` | Filesystem tree that packageR browses. Share paths are resolved inside this root. In production this is usually the folder where an object-storage bucket is mounted. |
...
| `FB_CREATE_USER_DIR` | Whether packageR should create generated user directories under the configured user-home base path. Defaults to `true` in `init.sh`; the settings default user-home base path is `/home`. |

## Branding

| Variable | Description |
| --- | --- |
| `FB_BRANDING_NAME` | Custom application name shown in the UI. Defaults to `packageR`. |
| `FB_BRANDING_FILES` | Static branding files path. Defaults to `/package-r`. |
